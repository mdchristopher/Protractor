...
define(function (require, exports, module) {
  ...
  exports['test github issue #56'] = function (assert, util) {
    var map = new SourceMapGenerator({
      sourceRoot: 'http://',
      file: 'www.specs.com/foo.js'
    });
    map.addMapping({
      original: { line: 1, column: 1 },
      generated: { line: 2, column: 2 },
      source: 'www.specs.com/original.js'
    });
    map = new SourceMapConsumer(map.toString());

    var sources = map.sources;
    assert.equal(sources.length, 1);
    assert.equal(sources[0], 'http://www.specs.com/original.js');
  };

  exports['test github issue #43'] = function (assert, util) {
    var map = new SourceMapGenerator({
      sourceRoot: 'http://specs.com',
      file: 'foo.js'
    });
    map.addMapping({
      original: { line: 1, column: 1 },
      generated: { line: 2, column: 2 },
      source: 'http://cdn.specs.com/original.js'
    });
    map = new SourceMapConsumer(map.toString());
  ...
  exports['test absolute path, but same host sources'] = function (assert, util) {
    ...
    assert.equal(sources.length, 1,
                 'Should only be one source.');
    assert.equal(sources[0], 'http://specs.com/original.js',
  ...
  exports['test github issue #72, duplicate sources'] = function (assert, util) {
    ...
    assert.equal(pos.source, 'http://specs.com/source1.js');
    assert.equal(pos.line, 1);
    assert.equal(pos.column, 1);
    ...
    assert.equal(pos.source, 'http://specs.com/source3.js');
  ...
  exports['test SourceMapConsumer.fromSourceMap'] = function (assert, util) {
    ...
    assert.equal(smc.sourceRoot, 'http://specs.com/');
    assert.equal(smc.sources.length, 2);
    assert.equal(smc.sources[0], 'http://specs.com/bar.js');
    assert.equal(smc.sources[1], 'http://specs.com/baz.js');
    ...
    assert.equal(pos.line, 1);
    assert.equal(pos.column, 1);
    assert.equal(pos.source, 'http://specs.com/bar.js');
    assert.equal(pos.name, null);

    pos = smc.generatedPositionFor({
      line: 1,
      column: 1,
      source: 'http://specs.com/bar.js'
    });
    assert.equal(pos.line, 2);
    assert.equal(pos.column, 2);
    ...
    assert.equal(pos.line, 2);
    assert.equal(pos.column, 2);
    assert.equal(pos.source, 'http://specs.com/baz.js');
    assert.equal(pos.name, 'dirtMcGirt');

    pos = smc.generatedPositionFor({
      line: 2,
      column: 2,
      source: 'http://specs.com/baz.js'
    });
